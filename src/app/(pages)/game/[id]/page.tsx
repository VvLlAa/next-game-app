...
export async function generateStaticParams() {
    ...
    const popularNewProductIds = res?.popularNewProducts?.map((card: GameType) => ({
        id: card.id,
    })) || [];

    const bestsellerProductIds = res?.bestsellerProducts?.map((card: GameType) => ({
        id: card.id,
    })) || [];

    const steamDeck = resSteamDeck?.games?.map((card: GameType) => ({
        id: card.id,
    })) || [];
